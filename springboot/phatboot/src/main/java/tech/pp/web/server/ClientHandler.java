...
public class ClientHandler extends Thread {
    ...
    public void run() {
        ...
        try (var inputStream = socket.getInputStream()) {
            ...
            try {
                var result = (HttpResponse) handler.getMethod().invoke(handler.getBean());
                String resultBody = "";

                switch (result.getResponseHeader().getContentType().toLowerCase()) {
                    case "text/json":
                        resultBody = objectMapper.writeValueAsString(result.getResponseBody());
                        break;
                    case "text/plain":
                        resultBody = (String) result.getResponseBody().toString();
                        break;
                    default:
                        break;
                }
                ...
                BuildHttpResponse response = new BuildHttpResponse(
                        result.getResponseHeader().getStatusCode(),
                        result.getResponseHeader().getContentType(),
                        String.valueOf(resultBody.length()));

                socket.getOutputStream().write(response.buildResponse(resultBody).getBytes());
